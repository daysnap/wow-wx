...
type PromisifyResult<P, T extends IAnyOne> = P extends { success: any } ? void : P extends { fail: any } ? void : P extends { complete: any } ? void : Promise<T>

export function promisify<T, P> (fn: string | ((options: T) => void)) {
  return function (options: T = {} as T) : PromisifyResult<T, P> {
    ...
    return result as PromisifyResult<T, P>
  }
}
